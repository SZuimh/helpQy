...
export default class MyHomerItem extends  Component{
    ...
    goPayForHomer(){
        this.props.navigation.navigate('PagePayForHomer',{HelpTypeMessage:this.props.row,getNewHomerListCallBack:this.props.getNewHomerListCallBack});
    }
        ...
        return(
            <View style={styles.PageMyHomersMaxView}>
                <View style={styles.PageMyHomersPlansView}>
                    <Text style={{fontSize:15,fontWeight:'bold',color:'black'}}>{this.getCategoryType(this.props.row.categorytype)}</Text>
                </View>
                <View style={styles.PageMyHomersContent}>
                    <Text style={{fontSize:13,color:'#4a4a4a',fontWeight:'bold'}}>{this.props.row.username}</Text>
                    <Text style={{fontSize:13,color:'#4a4a4a',fontWeight:'bold'}}>等待期{this.getLagTime(this.props.row.joindate)}天</Text>
                    <Text style={{fontSize:13,color:'#4a4a4a',fontWeight:'bold'}}>余额￥{this.state.paytotalmoney}</Text>
